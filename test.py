...
def edits1(word):
    ...
    swap_adjacent = [L + R[1] + R[0] + R[2:] for L, R in splits if len(R) > 1]
    return set(deletes + transposes + replaces + inserts + move_first_to_end + move_last_to_beginning + change_case + swap_adjacent)
...
def correct_sentence(sentence, error_count, trigram_freq, bigram_freq, unigram_freq, vocab):
        ...
        if word in vocab or not word.isalpha():  # 保留标点符号或非字母字符
            ...
            if candidate_list:
                if i > 1:
                    ...
                    best_candidate = max(candidate_list, key=lambda w: trigram_probability(trigram_freq, bigram_freq, previous_two_words[0], previous_two_words[1], w) * bigram_probability(bigram_freq, previous_two_words[1], w))
                elif i == 1:
                    previous_word = corrected_sentence[-1]
                    best_candidate = max(candidate_list, key=lambda w: bigram_probability(bigram_freq, previous_word, w))
                else:
    ...
    if real_word_count > 0:
        prob = []
        for i, word in enumerate(words):
            if word in non_word_errors or not word.isalpha() or word == "'s":  # 排除非单词错误、非字母字符以及 "'s"
            ...
            if i == 0:
                ...
                probability = trigram_probability(trigram_freq, bigram_freq, previous_two_words[0], previous_two_words[1], word)
